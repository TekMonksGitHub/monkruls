/** 
 * main.js, Main rules engine process. 
...
function _runRulesBundleOnData(rules, __internal_objects, __internal_data) {  // yes eval is bad in browser, but here it really is useful plus keeps variables function scoped, var is important below, can't use let or const
    ...
    try {
        ...
        for (const rule of rules) {
            ...
            else for (const else_doThis of rule_else_do) eval(else_doThis); 
        }
    } catch (err) {
...
async function runRules(input, current_folder) {
    const parseData = {current_folder: current_folder.split(path.sep).join(path.posix.sep)}; 
    ...
    let repeatLoop = false, loops = 0;
    do {
        const objectsCopy = CLONE_OBJECT(objects); loops++;
        for (const ruleBundle of ruleBundles) _runRulesBundleOnData(ruleBundle, objectsCopy, data);
