...
async function _parseCSV(path, isLookupTable, rawParse) {
    const parsedCSV = csvparser.parse(await fspromises.readFile(path, "utf-8"), {header: true, skipEmptyLines: true, 
        delimiter: ",", transformHeader: h=>h.trim()});
...
async function _parseObjects(objectDefinitions) {
    ...
    for (const objectDefinition of objectDefinitions) {
        const objToParse = Object.values(objectDefinition)[0].trim(), key = Object.keys(objectDefinition)[0].trim(), functionRE = objToParse.match(/(.+)\((.+)\)/);
        if (typeof objToParse == "string" && objToParse.toLowerCase().endsWith(".json")) objects[key] = JSON.parse(await fspromises.readFile(objToParse));
        else if (typeof objToParse == "string" && objToParse.toLowerCase().endsWith(".lookup_table.csv")) objects[key] = await _parseCSV(objToParse, true);
        else if (typeof objToParse == "string" && objToParse.toLowerCase().endsWith(".csv")) objects[key] = await _parseCSV(objToParse);
...
function _parseDecisionTable(rows, data) {    // parses a decision table into a regular rules bundle
    const makeCondition = (header, statement) => { let parsedCondition; for (let condition of statement.split(",")) {
        condition = condition.indexOf("$")!=-1?condition.replace(/\$/g, header):header+condition;
        parsedCondition = parsedCondition?parsedCondition+`&&(${condition})`:`(${condition})`;
    } return parsedCondition; }
    const ruleBundle = []; for (const row of rows) { const rule = {}; ruleBundle.push(rule); for (const header of Object.keys(row)) {
        const doRE = header.match(/.+\((.+)\).*/); if (doRE) rule.do = `${doRE[1]}="${row[header]}"`;
...
async function runRules(input, current_folder) {
    ...
    input = JSON.parse(mustache.render(JSON.stringify(input), parseData)); await coreRuleFuncs.init(input.functions||[]);

    let objects = await _parseObjects(input.objects), data = await _parseObjects(input.data);
    const rule_params = {}; for (const param of input.rule_parameters) rule_params[param.name] = param.value;
    const ruleBundles = []; for (const rule of input.rule_bundles) {
        if (typeof rule == "string" && rule.endsWith(".json")) ruleBundles.push(JSON.parse(mustache.render(await fspromises.readFile(rule, "utf8"), rule_params)));
        else if (typeof rule == "string" && rule.endsWith(".decision_table.csv")) ruleBundles.push(_parseDecisionTable(await _parseCSV(rule, false, true), objects));
		else if (rule instanceof Object && rule.rules) ruleBundles.push(rule.rules);
    }
