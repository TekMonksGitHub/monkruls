...
async function runRules(input, current_folder) {
    ...
    const rule_params = {}; for (const key in input.rule_parameters) rule_params[key] = input.rule_parameters[key].value;
    const ruleBundles = []; for (const rule of input.rule_bundles) {
        if (rule.endsWith(".json")) ruleBundles.push(JSON.parse(mustache.render(await fspromises.readFile(rule, "utf8"), rule_params)));
        else if (rule.endsWith(".decision_table.csv")) ruleBundles.push(_parseDecisionTable(await _parseCSV(rule, false, true), objects));
    }
