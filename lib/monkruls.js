...
function _parseDecisionTable(rows) {    // parses a decision table into a regular rules bundle
...
async function runRules(input, current_folder) {
    ...
    const ruleBundles = []; for (const rule of input.rule_bundles) {
        ...
        else if (rule.endsWith("table.csv")) ruleBundles.push(_parseDecisionTable(await _parseCSV(rule, false, true)));
    }
