...
async function _parseObjects(objectDefinitions) {
    ...
    for (const objectDefinition of objectDefinitions) {
        ...
        const key = objectDefinition.name.trim(), functionRE = objToParse.match?.(/(.+)\((.+)\)/);
...
async function runRules(input, current_folder="") {
    ...
    const ruleBundles = []; for (const rulebundle of parsedInput.rule_bundles) {
        if (typeof rulebundle.rules == "string" && utils.isValidURI(rulebundle.rules) && rulebundle.rules.endsWith(".json")) ruleBundles.push(JSON.parse(mustache.render(await fspromises.readFile(rulebundle.rules, "utf8"), rule_params)));
        else if (typeof rulebundle.rules == "string" && utils.isValidURI(rulebundle.rules) && rulebundle.rules.endsWith(".decision_table.csv")) ruleBundles.push(parseDecisionTable(await parseCSV(rulebundle.rules, false, true, true, rule_params), objects));
        ...
        }; ruleBundles.push(rulesThisBundle); }
    }
    ...
    if (failedRules.length) { LOG.info("Information on failed rules follows"); 
        for (const failedRule of failedRules) if (failedRule.reason) { LOG.info(failedRule.reason); failedReasons.push({"reason":failedRule.reason}); } }
