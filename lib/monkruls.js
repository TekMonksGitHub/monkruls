...
async function _parseObjects(objectDefinitions, additionalData={}) {
    ...
    for (const objectDefinition of objectDefinitions) {
        ...
        else if (typeof objToParse == "string" && functionRE) objects[key] = coreRuleFuncs[functionRE[1].trim()](...[...functionRE[2].trim().split(","), objects, additionalData]);
...
function parseDecisionTable(rows, data, valsGenerated=[]) {    // parses a decision table into a regular rules bundle
    const appendDoClause = (rule, lhs, rhs) => {
        if (rule.do) rule.do.push(`${lhs}=${utils.getAsJSExpression(rhs)}`); else rule.do = [`${lhs}=${utils.getAsJSExpression(rhs)}`];
        rule.doObjects[`${lhs}=${utils.getAsJSExpression(rhs)}`] = {lhs, rhs};
    }, valsGeneratedOriginal = CLONE_OBJECT(valsGenerated);

    const makeCondition = (header, statement) => statement.indexOf("$")!=-1?statement.replace(/\$/g, header):header+statement;
    const ruleBundle = []; for (const row of rows) { const rule = {isDecisionTableRule: true, doObjects: {}}; ruleBundle.push(rule); for (const header in row) { 
        if (!row[header].trim().length) continue;
        const doRE = header.match(/.+\((.+)\).*/); 
        if (doRE) appendDoClause(rule, doRE[1], row[header]); 
        else if (header.trim().toLowerCase() == "reason") rule.reason = row[header]
        else if (!utils.getObjectValue(header, data) && (!valsGeneratedOriginal.includes(header))) {
            appendDoClause(rule, header, row[header]); if (!valsGenerated.includes(header)) valsGenerated.push(header); }
...
function _runRulesBundleOnData(rules, __internal_objects, __internal_data, bundleName) {  // yes eval is bad in browser, but here it really is useful plus keeps variables function scoped, var is important below, can't use let or const
    ...
    try {
        ...
        for (const [index, rule] of rules.entries()) {
            ...
            if ($condition_value) for (const doThis of rule_do) try {eval(doThis);} catch (err) {
                if (rule.isDecisionTableRule) eval(`${rule.doObjects[doThis].lhs}=${JSON.stringify(rule.doObjects[doThis].rhs)}`); else throw err;}
...
async function runRules(input, current_folder="") {
    ...
    const parsedInput = JSON.parse(utils.preserveUnresolvableTagsMustacheRender(JSON.stringify(input), parseData)); 
    await coreRuleFuncs.init(parsedInput.functions||[]);

    let objects = await _parseObjects(parsedInput.objects), data = await _parseObjects(parsedInput.data, objects);
    const rule_params = {}; for (const param of parsedInput.rule_parameters) rule_params[param.name] = param.value;
    const ruleBundles = [], dtValsGenerated = []; for (const rulebundle of parsedInput.rule_bundles) {
        if (typeof rulebundle.rules == "string" && utils.isValidURI(rulebundle.rules) && rulebundle.rules.endsWith(".json")) ruleBundles.push({name:rulebundle.name, rules:JSON.parse(mustache.render(await fspromises.readFile(rulebundle.rules, "utf8"), rule_params))});
        else if (typeof rulebundle.rules == "string" && utils.isValidURI(rulebundle.rules) && rulebundle.rules.endsWith(".decision_table.csv")) ruleBundles.push({name:rulebundle.name, rules:parseDecisionTable(await parseCSV(rulebundle.rules, false, true, true, rule_params), objects, dtValsGenerated)});
            ...
            if (rule.condition) rulesThisBundle.push(JSON.parse(mustache.render(JSON.stringify(rule), rule_params)));   // simple rules array
                ...
                await parseCSV(rule.decisiontable.substring(CSVSCHEME.length), false, true, true, rule_params), objects, dtValsGenerated)); // inline CSV for decision table
