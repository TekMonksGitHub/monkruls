...
const _flattenObjArray = array => {const flatArray = []; for (const object of array) {
        const flatObj = {}; for (const key in object) flatObj[key] = object[key] instanceof Array?JSON.stringify(object[key]):object[key]; flatArray.push(flatObj);
    }; return flatArray; }
const _objectToCSV = object => csvparser.unparse(_flattenObjArray(object), {header: true, delimiter: ","});
