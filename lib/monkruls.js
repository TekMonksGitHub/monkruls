...
async function _parseCSV(path, isLookupTable, rawParse, renderWithMustache, dataForMustache={}) {
    const csvContents = renderWithMustache ? mustache.render(utils.isValidURI(path)?await fspromises.readFile(path, "utf-8"):path, dataForMustache) :  utils.isValidURI(path)?await fspromises.readFile(path, "utf-8"):path;
...
async function runRules(input, current_folder) {
    ...
    const ruleBundles = []; for (const rule of parsedInput.rule_bundles) {
        ...
        else if (typeof rule == "string" && rule.endsWith(".decision_table.csv")) ruleBundles.push(_parseDecisionTable(await _parseCSV(rule, false, true, true, rule_params), objects));
		else if (rule instanceof Object && rule.rules && Array.isArray(rule.rules)) ruleBundles.push(rule.rules);   // simple rules array
        else if (rule instanceof Object && rule.rules && typeof rule.rules == "string") ruleBundles.push(_parseDecisionTable(await _parseCSV(rule.rules, false, true, true, rule_params), objects)); // inline CSV for decision table
    }
