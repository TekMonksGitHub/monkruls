...
/**
 * This function expands the given Mustache tags using default tag expressions "{{" and "}}"
 * but will not destroy unresolvable tags with empty strings.
 * @param {string} template The template
 * @param {obiect} data The data
 */
function preserveUnresolvableTagsMustacheRender(template, data) {
    let returnString = new String(template); 
    for (const key in data) { const re = new RegExp(`{{{${key}}}}`, "g"); returnString = returnString.replace(re, data[key]); }
    for (const key in data) { const re = new RegExp(`{{${key}}}`, "g"); returnString = returnString.replace(re, data[key]); }
    return returnString.toString();
}

module.exports = { parseBoolean, getDateTime, getTimeStamp, areObjectsSame, getObjectValue, requireAsync, 
    isValidURI, flattenObjArray, getAsJSExpression, preserveUnresolvableTagsMustacheRender };